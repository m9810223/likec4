...
/**
 * Capture each view as SVG under the output directory using the provided browser context.
 * Reads `window.__LIKEC4_SVG_DATA` (a data URL set by the frontend) and decodes it to SVG content.
...
export async function captureSvg({
  ...
  while ((next = pending.shift())) {
    ...
    try {
      ...
      const path = resolve(output, relativePath, `${view.id}.svg`)
